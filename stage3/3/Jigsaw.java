...
public class Jigsaw {
		...
		while (!openList.isEmpty()) {
			...
			if (!currentJNode.equals(endJNode)) {
				...
				for (int i = 0 ; i < 4 ; i++) {
					...
					if (temp.move(i) && !closeList.contains(temp)) {
						openList.addElement(temp);
					}
				}
				searchedNodesNum++;
			} else {
	 ...
	 * @throws IOException
	 */
	public int ASearch() throws IOException{
		...
		while (this.openList.isEmpty() != true && searchedNodesNum <= maxNodesNum) {
			...
			System.out.println("YS-value: " + currentJNode.getEstimatedValue() + "\n" + "YS-deep: " + currentJNode.getNodeDepth() + 
					"\nYS-inclose? : " + closeList.contains(currentJNode));
			...
			pw.println("估值 = " + this.getCurrentJNode().getEstimatedValue());
		}
		
		this.printResult(pw);	// 记录搜索结果
		pw.close(); 			// 关闭输出文件
		System.out.println("Record into " + filePath);
		return this.getSearchedNodesNum();
	}
	...
	private void estimateValue(JigsawNode jNode) {
		int s0 = 0; // 上续节点不正确的数码个数
		int s1 = 0; // 右点不正确的数码个数
		int s2 = 0; // 下续节点不正确的数码个数
		int s3 = 0; // 左续节点不正确的数码个数
		int wrongNum = 0; //放错位置节点数量
		double wrongMDis = 0; // 放错距离之和
		int wrongDis = 0; // 放错M距离之和
		int dimen = 5;
		int dimension = JigsawNode.getDimension();
		for(int index =1 ; index<dimension*dimension; index++){
			if(index > dimen  && jNode.getNodesState()[index-dimen] != jNode.getNodesState()[index] - dimen || 
					index > dimen && jNode.getNodesState()[index] <= dimen) {
				s0++;
			}
			...
			if(jNode.getNodesState()[index] != index && jNode.getNodesState()[index] !=0) {
				//wrongNum = wrongNum + 10 - index/dimen - index % dimen;
				//wrongNum++;
				wrongMDis += getMDistance(jNode.getNodesState()[index], index);
				wrongDis += getDistance(jNode.getNodesState()[index], index);
			}
		}
		int index = 25;
		...
		double value = 8*s1 +  + 5*wrongMDis +4*wrongDis;
		//744-5000
		jNode.setEstimatedValue((int)value);
		System.out.println("估值 = " + value);
	}
	private int getMDistance(int a, int b) {
		/*if (a == 0)
			a = 25;*/
		int dimen = 5;
		int c =Math.abs((a-1)/dimen - (b-1)/dimen);
		int d =Math.abs((a-1)%dimen  - (b-1)%dimen);
		//System.out.println("曼哈顿距离: " + a + " + " + b + " = " +  (c+d));
		return c+d;
	}
	private double getDistance(int a, int b) {
		...
		int c =Math.abs((a-1)/dimen - (b-1)/dimen);
		int d =Math.abs((a-1)%dimen - (b-1)%dimen );
		//System.out.println("几何距离: " + a + " + " + b + " = " +  Math.sqrt(c*c + d*d));
